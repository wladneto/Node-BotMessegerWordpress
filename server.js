...
var sockets = [];
var _estados =[];
var logo = 'https://i2.wp.com/www.nos2demochilao.com.br/wp-content/uploads/2017/02/04-e1486690093189.png'; //logodosite
//Inicio do ROBO

router.get('/webhook',function (requisicao, resposta) {
  
  if (requisicao.query['hub.mode'] === 'subscribe' && requisicao.query['hub.verify_token'] === 'senhaABC') {
...
router.post('/webhook', function (requisicao,resposta) {
...
function buscarnomeparamenu (recipientID){
  ...
  var usuario = "https://graph.facebook.com/v2.6/"+recipientID+"?fields=first_name,last_name,profile_pic,locale,timezone,gender&access_token=EAAITd2revG4BAJGVsozHE4oimHr16kYtdLoZAfygxSEEpt0a7fLpUdNH8GbkW5DZBS8fAxvFClF326TuOOAIDylzHYmSieSXUN0j7rYZBDVZAZAc4ty9DbTbS9dL296PK1ZAvlOON90PthcKjhDEdRdX05YnJUFTKFI7YLDSSwgwZDZD";    
...
function callSendAPI (messageData) {
  request({
    ...
    qs: { access_token: 'EAAITd2revG4BAJGVsozHE4oimHr16kYtdLoZAfygxSEEpt0a7fLpUdNH8GbkW5DZBS8fAxvFClF326TuOOAIDylzHYmSieSXUN0j7rYZBDVZAZAc4ty9DbTbS9dL296PK1ZAvlOON90PthcKjhDEdRdX05YnJUFTKFI7YLDSSwgwZDZD' },
    method: 'POST',
    json: messageData 
    }, function (error, respose, body) {
      if (!error && respose.statusCode == 200){
...
setTimeout(function() {
sendTextMessage(recipientID, 'Ok mochileiro... ;)          Todos os nossos roteiros prontos possuem as planilhas em excel para o seu planejamento.');
},100);
setTimeout(function() {
sendTextMessage(recipientID, 'Confira os roteiros prontos deslizando para esquerda ⬅️ ');
},2000);

// TENHO Q COLOCAR O DIGITANDO AQUI
if (saiudaconsulta == true) {
sendRoteirosPosts(recipientID,titulo,imagem,url,urlpronta); 
} else {
setTimeout(function() {
sendRoteirosPosts(recipientID,titulo,imagem,url,urlpronta); 
  }, 5000);
...
//textos
setTimeout(function() {
sendTextMessage(recipientID, 'Todos os roteiros prontos possuem:');
  setTimeout(function() {
  sendTextMessage(recipientID, '📍 Separação de gastos por tipos: Hospedagem 🏢, Alimentação🍝, Transportes✈️ e Atrações🗿');
    setTimeout(function() {
    sendTextMessage(recipientID, '📍 Separação de gastos pelos países, quando o roteiro contempla mais de um país ;)');
      setTimeout(function() {
      sendTextMessage(recipientID, '📍 Estão vinculados as moedas 💵💴💶 de cada país para o controle e entendimento das alterações cambiais durante seu planejamento.');
        setTimeout(function() {
        sendTextMessage(recipientID, '📍 Além disso sugerimos alguns hostels, pousadas e hotéis.');
        },2000);
      },2000);
    },2000);
  },2000);
...
//
console.log('SAIU DA FUNCAO CONSULTA POST ROTEIROS');
     
} 

function ConsultarPostDicas (recipientID) {
  console.log('ENTROU NA FUNCAO CONSULTA POST DICAS');
    var titulo=[];
    var imagem=[];
    var url=[];
    var saiudaconsulta = false;
    ...
    //chamar apenas os roteiros
    var options = { method: 'GET',
          url: 'http://www.nos2demochilao.com.br/wp-json/wp/v2/listing-item/?per_page=10&&listing-item-category=598',
      };
    request(options, function (error, response, body) {
    if (error) throw new Error(error);
...
setTimeout(function() {
sendTextMessage(recipientID, 'Ok mochileiro... ;)              Confira nossas dicas deslizando para esquerda ⬅️ ️');
},100);

// TENHO Q COLOCAR O DIGITANDO AQUI
if (saiudaconsulta == true) {
sendDicasPosts(recipientID,titulo,imagem,url); 
} else {
setTimeout(function() {
sendDicasPosts(recipientID,titulo,imagem,url); 
  }, 5000);
...
//FIM DAS CONSULTAS NO SITE


// primeira pagina para baixo
...
server.listen(process.env.PORT || 3000, process.env.IP || "0.0.0.0", function(){
  ...
  console.log("Servico iniciado", addr.address + ":" + addr.port);
});
